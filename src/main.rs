...
// mod libavcodec;
// use crate::libavcodec::decode_packet;
use std::{
    ffi::{CStr, CString, c_int}, ptr, slice,
};

unsafe extern "C" {
    fn decode_packet(
        pPacket: *mut ffi::AVPacket,
...
fn main() {
    let filepath: CString = CString::new("./src/bear.mp4").unwrap();
    ...
    while unsafe { ffi::av_read_frame(format_context, packet) } >= 0 {
        if video_stream_index == Some(packet.stream_index as usize) {
            ...
            unsafe {
             decode_packet(packet, codec_context, frame);   
            }
            packets_waiting -= 1;
